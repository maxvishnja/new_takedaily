...
<div style="width: 100%; height: 230mm; padding: 0;position: relative;">
	...
	<div style="font-weight: 200;font-size: 7pt;color: #1A8562;line-height: 1;position: absolute; bottom: 0; text-align: center">
		...
		@endif
		<strong>{{ trans('label-product.recommended-daily-use') }}</strong>
			{!! trans('label-product.info') !!}<br/>
			{!! trans('label-product.recommended-daily-use') !!} <strong>{!! trans('label-product.amount-daily') !!}</strong> {!! trans('label-product.when-to-take') !!} {!! trans('label-product.health-disclaimer') !!} <strong>{!! trans('label-product.max-limit') !!}</strong>
