	...
	$.fn.liKnopik = function(params){
		...
		return this.each(function(){
			...
			knopikBut.on('click',function(){
				...
				//		knopik.removeClass('hiddens').css({right:-knopikW,left:'auto'}).animate({right:'0'});
				//	}else{
				//		$.cookie(knopikIndex,'false');
				//		knopik.addClass('hiddens').css({right:0,left:'auto'}).animate({right:-knopikW});
				//	}
				//}else{
				//	if(knopik.is('.hiddens')){
				//		$.cookie(knopikIndex,'true');
				//		knopik.removeClass('hiddens').css({left:-knopikW,right:'auto'}).animate({left:'0'});
				//	}else{
				//		$.cookie(knopikIndex,'false');
				//		knopik.addClass('hiddens').css({right:'auto',left:0}).animate({left:-knopikW});
				//	}
				//}
			});
		});
