...
class CreateCsvMonths
{
    public static function storeAllCustomerToCsv($index,$customers, $lang)
    {
        ...
        \Log::info('Start in function create month CSV ' . $lang);
        $email_array = [];
        $i = 0;
        ...
        \Excel::create('all_users_month_' . $lang . "_" . $index, function ($excel) use ($email_array) {
            $excel->sheet('All users', function ($sheet) use ($email_array) {
                $sheet->fromArray($email_array, null, 'A1', true);
            });
        })->store('xls', storage_path('excel/exports/month/' . $lang));

        $cache = \Cache::get('csv_current_index_month');

        \Cache::put('csv_current_index_month', $cache+1, 10);

        \Log::info('Cahce ' . \Cache::get('csv_current_index'));
        if(\Cache::get('csv_current_index_month') == \Cache::get('csv_total_month')){
            ...
            try {
                ...
                $files = glob(storage_path('excel/exports/month/' . $lang . '/*'));
                \Zipper::make(storage_path('excel/exports/all_users_month_' . $lang . '.zip'))->add($files)->close();
                \File::deleteDirectory(storage_path('excel/exports/month/' . $lang));

                \Cache::put('csv_current_index_month',0);
