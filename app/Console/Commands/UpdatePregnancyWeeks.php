...
class UpdatePregnancyWeeks extends Command
{
	...
	{
		$customersWithBirthdates = Customer::select( 'customers.*', 'customer_attributes.value as pregnancy_date' )
		                                   ->leftJoin( 'customer_attributes', 'customer_attributes.customer_id', '=', 'customers.id' )
		                                   ->where( 'customer_attributes.identifier', 'user_data.pregnancy.date' )
		                                   ->get();

		foreach ( $customersWithBirthdates as $customer )
		{
			if ( $customer->pregnancy_date != '' )
			{
